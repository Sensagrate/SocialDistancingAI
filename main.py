...
# The following module is added for json functionality
import json 
...
data = []
num_six_feet_violations = stay_at_home_index_percent = social_distancing_index_percent = 0

data_dict = {}
data_dict[measurement_name] = []
...
while cap.isOpened():
    ...
    data_dict[measurement_name].append({
        'location': location_tags,
        'num_six_feet_violations': str(np.round(num_six_feet_violations)),
        'stay_at_home_index_percent': str(stay_at_home_index_percent),
        'social_distancing_index_percent': str(social_distancing_index_percent),
    ...
    client.write_points(data, database='Social-Distancing-AI', time_precision='n', batch_size=500, protocol='line')
...
with open('social-distancing-ai.json', 'w') as json_file:
